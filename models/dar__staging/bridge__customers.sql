MODEL (
  ...
  description 'Bridge viewpoint of customer data: Current customer information. Also see the Person and Store tables.',
  column_descriptions (
    ...
    _hook__customer = 'Primary hook to customer',
...
cte__pit_lookup AS (
  SELECT
    ...
    bridge__stores._pit_hook__person__sales,
    bridge__stores._pit_hook__reference__country_region,
    bridge__stores._pit_hook__store,
    bridge__stores._pit_hook__territory__sales,
    cte__bridge._hook__customer,
    GREATEST(
        cte__bridge.bridge__record_loaded_at,
        bridge__stores.bridge__record_loaded_at,
        bridge__sales_territories.bridge__record_loaded_at
    ) AS bridge__record_loaded_at,
    GREATEST(
        cte__bridge.bridge__record_updated_at,
        bridge__stores.bridge__record_updated_at,
        bridge__sales_territories.bridge__record_updated_at
    ) AS bridge__record_updated_at,
    GREATEST(
        cte__bridge.bridge__record_valid_from,
        bridge__stores.bridge__record_valid_from,
        bridge__sales_territories.bridge__record_valid_from
    ) AS bridge__record_valid_from,
    LEAST(
        cte__bridge.bridge__record_valid_to,
        bridge__stores.bridge__record_valid_to,
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          bridge__stores.bridge__is_current_record,
          bridge__sales_territories.bridge__is_current_record
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.bridge__stores
  ON cte__bridge._hook__store = bridge__stores._hook__store
  AND cte__bridge.bridge__record_valid_from >= bridge__stores.bridge__record_valid_from
  AND cte__bridge.bridge__record_valid_to <= bridge__stores.bridge__record_valid_to
