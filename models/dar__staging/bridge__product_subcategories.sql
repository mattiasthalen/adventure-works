MODEL (
  ...
  description 'Bridge viewpoint of product_subcategory data: Product subcategories. See ProductCategory table.',
  column_descriptions (
    ...
    _hook__product_subcategory = 'Primary hook to product_subcategory',
...
cte__pit_lookup AS (
  SELECT
    ...
    bridge__product_categories._pit_hook__product_category,
    cte__bridge._hook__product_subcategory,
    GREATEST(
        cte__bridge.bridge__record_loaded_at,
        bridge__product_categories.bridge__record_loaded_at
    ) AS bridge__record_loaded_at,
    GREATEST(
        cte__bridge.bridge__record_updated_at,
        bridge__product_categories.bridge__record_updated_at
    ) AS bridge__record_updated_at,
    GREATEST(
        cte__bridge.bridge__record_valid_from,
        bridge__product_categories.bridge__record_valid_from
    ) AS bridge__record_valid_from,
    LEAST(
        cte__bridge.bridge__record_valid_to,
        bridge__product_categories.bridge__record_valid_to
    ) AS bridge__record_valid_to,
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          cte__bridge.bridge__is_current_record,
          bridge__product_categories.bridge__is_current_record
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.bridge__product_categories
  ON cte__bridge._hook__product_category = bridge__product_categories._hook__product_category
  AND cte__bridge.bridge__record_valid_from >= bridge__product_categories.bridge__record_valid_from
  AND cte__bridge.bridge__record_valid_to <= bridge__product_categories.bridge__record_valid_to
),
