MODEL (
  ...
  description 'Event viewpoint of address_types data: Types of addresses stored in the Address table.',
  column_descriptions (
    peripheral = 'Name of the address_types peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__address_types
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__address_type,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'address_type__modified_date' THEN 1 END) AS event__address_types_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__address_type)
)
SELECT
  peripheral::TEXT,
