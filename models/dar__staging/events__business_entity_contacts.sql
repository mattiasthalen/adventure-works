MODEL (
  ...
  description 'Event viewpoint of business_entity_contacts data: Cross-reference table mapping stores, vendors, and employees to people.',
  column_descriptions (
    peripheral = 'Name of the business_entity_contacts peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__business_entity_contacts
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__business_entity,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'business_entity_contact__modified_date' THEN 1 END) AS event__business_entity_contacts_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__business_entity)
)
SELECT
  peripheral::TEXT,
