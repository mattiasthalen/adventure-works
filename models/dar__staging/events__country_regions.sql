MODEL (
  ...
  description 'Event viewpoint of country_regions data: Lookup table containing the ISO standard codes for countries and regions.',
  column_descriptions (
    peripheral = 'Name of the country_regions peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__country_regions
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__country_region,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'country_region__modified_date' THEN 1 END) AS event__country_regions_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__country_region)
)
SELECT
  peripheral::TEXT,
