MODEL (
  ...
  description 'Event viewpoint of cultures data: Lookup table containing the languages in which some AdventureWorks data is stored.',
  column_descriptions (
    peripheral = 'Name of the cultures peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__cultures
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__culture,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'culture__modified_date' THEN 1 END) AS event__cultures_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__culture)
)
SELECT
  peripheral::TEXT,
