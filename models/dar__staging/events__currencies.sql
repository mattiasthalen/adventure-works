MODEL (
  ...
  description 'Event viewpoint of currencies data: Lookup table containing standard ISO currencies.',
  column_descriptions (
    peripheral = 'Name of the currencies peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__currencies
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__currency,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'currency__modified_date' THEN 1 END) AS event__currencies_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__currency)
)
SELECT
  peripheral::TEXT,
