...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__currency_rates
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__currency_rate,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'currency_rate__currency_rate_date' THEN 1 END) AS event__currency_rates_currency_rate,
    MAX(CASE WHEN pivot__events.event = 'currency_rate__modified_date' THEN 1 END) AS event__currency_rates_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__currency_rate)
)
SELECT
  peripheral::TEXT,
