MODEL (
  ...
  description 'Event viewpoint of customers data: Current customer information. Also see the Person and Store tables.',
  column_descriptions (
    peripheral = 'Name of the customers peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__customers
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__customer,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'customer__modified_date' THEN 1 END) AS event__customers_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__customer)
)
SELECT
  peripheral::TEXT,
