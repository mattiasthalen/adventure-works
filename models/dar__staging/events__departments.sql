MODEL (
  ...
  description 'Event viewpoint of departments data: Lookup table containing the departments within the Adventure Works Cycles company.',
  column_descriptions (
    peripheral = 'Name of the departments peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__departments
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__department,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'department__modified_date' THEN 1 END) AS event__departments_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__department)
)
SELECT
  peripheral::TEXT,
