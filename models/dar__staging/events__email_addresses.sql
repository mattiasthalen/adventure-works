MODEL (
  ...
  description 'Event viewpoint of email_addresses data: Where to send a person email.',
  column_descriptions (
    peripheral = 'Name of the email_addresses peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__email_addresses
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__person__individual,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'email_address__modified_date' THEN 1 END) AS event__email_addresses_modified
