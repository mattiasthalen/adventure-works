MODEL (
  ...
  description 'Event viewpoint of employees data: Employee information.',
  column_descriptions (
    peripheral = 'Name of the employees peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__employees
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__person__employee,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'employee__birth_date' THEN 1 END) AS event__employees_birth,
    MAX(CASE WHEN pivot__events.event = 'employee__hire_date' THEN 1 END) AS event__employees_hire,
    MAX(CASE WHEN pivot__events.event = 'employee__modified_date' THEN 1 END) AS event__employees_modified
