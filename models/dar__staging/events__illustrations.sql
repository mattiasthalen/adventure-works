MODEL (
  ...
  description 'Event viewpoint of illustrations data: Bicycle assembly diagrams.',
  column_descriptions (
    peripheral = 'Name of the illustrations peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__illustrations
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__illustration,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'illustration__modified_date' THEN 1 END) AS event__illustrations_modified
