MODEL (
  ...
  description 'Event viewpoint of locations data: Product inventory and manufacturing locations.',
  column_descriptions (
    peripheral = 'Name of the locations peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__locations
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__location,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'location__modified_date' THEN 1 END) AS event__locations_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__location)
)
SELECT
  peripheral::TEXT,
