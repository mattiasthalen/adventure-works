MODEL (
  ...
  description 'Event viewpoint of person_phones data: Telephone number and type of a person.',
  column_descriptions (
    peripheral = 'Name of the person_phones peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__person_phones
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__person__individual,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'person_phone__modified_date' THEN 1 END) AS event__person_phones_modified
