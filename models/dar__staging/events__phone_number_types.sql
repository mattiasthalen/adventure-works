MODEL (
  ...
  description 'Event viewpoint of phone_number_types data: Type of phone number of a person.',
  column_descriptions (
    peripheral = 'Name of the phone_number_types peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__phone_number_types
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__phone_number_type,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'phone_number_type__modified_date' THEN 1 END) AS event__phone_number_types_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__phone_number_type)
)
SELECT
  peripheral::TEXT,
