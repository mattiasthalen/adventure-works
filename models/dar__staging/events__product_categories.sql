MODEL (
  ...
  description 'Event viewpoint of product_categories data: High-level product categorization.',
  column_descriptions (
    peripheral = 'Name of the product_categories peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__product_categories
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__product_category,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'product_category__modified_date' THEN 1 END) AS event__product_categories_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__product_category)
)
SELECT
  peripheral::TEXT,
