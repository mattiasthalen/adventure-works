MODEL (
  ...
  description 'Event viewpoint of product_cost_histories data: Changes in the cost of a product over time.',
  column_descriptions (
    peripheral = 'Name of the product_cost_histories peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__product_cost_histories
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__product_cost_history,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'product_cost_history__start_date' THEN 1 END) AS event__product_cost_histories_started,
    MAX(CASE WHEN pivot__events.event = 'product_cost_history__end_date' THEN 1 END) AS event__product_cost_histories_ended,
    MAX(CASE WHEN pivot__events.event = 'product_cost_history__modified_date' THEN 1 END) AS event__product_cost_histories_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__product_cost_history)
)
SELECT
  peripheral::TEXT,
