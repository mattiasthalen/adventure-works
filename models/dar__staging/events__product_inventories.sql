MODEL (
  ...
  description 'Event viewpoint of product_inventories data: Product inventory information.',
  column_descriptions (
    peripheral = 'Name of the product_inventories peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__product_inventories
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__product_location,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'product_inventory__modified_date' THEN 1 END) AS event__product_inventories_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__product_location)
)
SELECT
  peripheral::TEXT,
