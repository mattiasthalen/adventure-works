MODEL (
  ...
  description 'Event viewpoint of product_reviews data: Customer reviews of products they have purchased.',
  column_descriptions (
    peripheral = 'Name of the product_reviews peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__product_reviews
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__product_review,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'product_review__review_date' THEN 1 END) AS event__product_reviews_review,
    MAX(CASE WHEN pivot__events.event = 'product_review__modified_date' THEN 1 END) AS event__product_reviews_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__product_review)
)
SELECT
  peripheral::TEXT,
