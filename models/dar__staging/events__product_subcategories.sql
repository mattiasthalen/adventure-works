MODEL (
  ...
  description 'Event viewpoint of product_subcategories data: Product subcategories. See ProductCategory table.',
  column_descriptions (
    peripheral = 'Name of the product_subcategories peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__product_subcategories
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__product_subcategory,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'product_subcategory__modified_date' THEN 1 END) AS event__product_subcategories_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__product_subcategory)
)
SELECT
  peripheral::TEXT,
