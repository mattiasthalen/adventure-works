MODEL (
  ...
  description 'Event viewpoint of sales_person_quota_histories data: Sales performance tracking.',
  column_descriptions (
    peripheral = 'Name of the sales_person_quota_histories peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__sales_person_quota_histories
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__person__sales,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'sales_person_quota_history__quota_date' THEN 1 END) AS event__sales_person_quota_histories_quota,
    MAX(CASE WHEN pivot__events.event = 'sales_person_quota_history__modified_date' THEN 1 END) AS event__sales_person_quota_histories_modified
