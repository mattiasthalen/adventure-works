MODEL (
  ...
  description 'Event viewpoint of sales_persons data: Sales representative current information.',
  column_descriptions (
    peripheral = 'Name of the sales_persons peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__sales_persons
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__person__sales,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'sales_person__modified_date' THEN 1 END) AS event__sales_persons_modified
