...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__sales_reasons
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__sales_reason,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'sales_reason__modified_date' THEN 1 END) AS event__sales_reasons_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__sales_reason)
)
SELECT
  peripheral::TEXT,
