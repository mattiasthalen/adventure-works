MODEL (
  ...
  description 'Event viewpoint of sales_territories data: Sales territory lookup table.',
  column_descriptions (
    peripheral = 'Name of the sales_territories peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__sales_territories
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__territory__sales,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'sales_territory__modified_date' THEN 1 END) AS event__sales_territories_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__territory__sales)
)
SELECT
  peripheral::TEXT,
