MODEL (
  ...
  description 'Event viewpoint of scrap_reasons data: Manufacturing failure reasons lookup table.',
  column_descriptions (
    peripheral = 'Name of the scrap_reasons peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__scrap_reasons
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__scrap_reason,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'scrap_reason__modified_date' THEN 1 END) AS event__scrap_reasons_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__scrap_reason)
)
SELECT
  peripheral::TEXT,
