MODEL (
  ...
  description 'Event viewpoint of shifts data: Work shift lookup table.',
  column_descriptions (
    peripheral = 'Name of the shifts peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__shifts
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__shift,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'shift__modified_date' THEN 1 END) AS event__shifts_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__shift)
)
SELECT
  peripheral::TEXT,
