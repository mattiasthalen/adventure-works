MODEL (
  ...
  description 'Event viewpoint of ship_methods data: Shipping company lookup table.',
  column_descriptions (
    peripheral = 'Name of the ship_methods peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__ship_methods
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__ship_method,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'ship_method__modified_date' THEN 1 END) AS event__ship_methods_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__ship_method)
)
SELECT
  peripheral::TEXT,
