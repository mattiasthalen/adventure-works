...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__special_offers
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__special_offer,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'special_offer__start_date' THEN 1 END) AS event__special_offers_started,
    MAX(CASE WHEN pivot__events.event = 'special_offer__end_date' THEN 1 END) AS event__special_offers_ended,
    MAX(CASE WHEN pivot__events.event = 'special_offer__modified_date' THEN 1 END) AS event__special_offers_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__special_offer)
)
SELECT
  peripheral::TEXT,
