MODEL (
  ...
  description 'Event viewpoint of state_provinces data: State and province lookup table.',
  column_descriptions (
    peripheral = 'Name of the state_provinces peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__state_provinces
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__state_province,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'state_province__modified_date' THEN 1 END) AS event__state_provinces_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__state_province)
)
SELECT
  peripheral::TEXT,
