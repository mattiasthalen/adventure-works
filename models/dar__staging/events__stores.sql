MODEL (
  ...
  description 'Event viewpoint of stores data: Customers (resellers) of Adventure Works products.',
  column_descriptions (
    peripheral = 'Name of the stores peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__stores
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__store,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'store__modified_date' THEN 1 END) AS event__stores_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__store)
)
SELECT
  peripheral::TEXT,
