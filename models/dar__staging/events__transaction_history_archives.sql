MODEL (
  ...
  description 'Event viewpoint of transaction_history_archives data: Transactions for previous years.',
  column_descriptions (
    peripheral = 'Name of the transaction_history_archives peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__transaction_history_archives
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__transaction_history_archive,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'transaction_history_archive__transaction_date' THEN 1 END) AS event__transaction_history_archives_transaction,
    MAX(CASE WHEN pivot__events.event = 'transaction_history_archive__modified_date' THEN 1 END) AS event__transaction_history_archives_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__transaction_history_archive)
)
SELECT
  peripheral::TEXT,
