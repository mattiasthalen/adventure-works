MODEL (
  ...
  description 'Event viewpoint of unit_measures data: Unit of measure lookup table.',
  column_descriptions (
    peripheral = 'Name of the unit_measures peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__unit_measures
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__reference__unit_measure,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'unit_measure__modified_date' THEN 1 END) AS event__unit_measures_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__reference__unit_measure)
)
SELECT
  peripheral::TEXT,
