MODEL (
  ...
  description 'Event viewpoint of vendors data: Companies from whom Adventure Works Cycles purchases parts or other goods.',
  column_descriptions (
    peripheral = 'Name of the vendors peripheral this event relates to',
...
WITH cte__bridge AS (
  ...
  FROM dar__staging.bridge__vendors
),
cte__events AS (
  SELECT
    pivot__events._pit_hook__vendor,
    CONCAT('epoch__date|', pivot__events.event_date) AS _hook__epoch__date,
    MAX(CASE WHEN pivot__events.event = 'vendor__modified_date' THEN 1 END) AS event__vendors_modified
...
final AS (
  ...
  FROM cte__bridge
  LEFT JOIN cte__events USING(_pit_hook__vendor)
)
SELECT
  peripheral::TEXT,
