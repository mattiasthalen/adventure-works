...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__address,
    business_entity_address__modified_date::DATE AS measure_date,
    1 AS measure__business_entity_addresses_modified
  FROM cte__source
  WHERE business_entity_address__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
