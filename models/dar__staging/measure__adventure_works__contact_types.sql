MODEL (
  ...
  tags hook,
  grain (_pit_hook__reference__contact_type, _hook__epoch__date),
  references (_pit_hook__reference__contact_type, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__reference__contact_type,
    contact_type__modified_date::DATE AS measure_date,
    1 AS measure__contact_types_modified
  FROM cte__source
  WHERE contact_type__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
