MODEL (
  ...
  tags hook,
  grain (_pit_hook__credit_card, _hook__epoch__date),
  references (_pit_hook__credit_card, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__credit_card,
    credit_card__modified_date::DATE AS measure_date,
    1 AS measure__credit_cards_modified
  FROM cte__source
  WHERE credit_card__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
