MODEL (
  ...
  tags hook,
  grain (_pit_hook__person__individual, _hook__epoch__date),
  references (_pit_hook__person__individual, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__person__individual,
    person_phone__modified_date::DATE AS measure_date,
    1 AS measure__person_phones_modified
  FROM cte__source
  WHERE person_phone__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
