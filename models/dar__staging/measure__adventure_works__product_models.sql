MODEL (
  ...
  tags hook,
  grain (_pit_hook__reference__product_model, _hook__epoch__date),
  references (_pit_hook__reference__product_model, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__reference__product_model,
    product_model__modified_date::DATE AS measure_date,
    1 AS measure__product_models_modified
  FROM cte__source
  WHERE product_model__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
