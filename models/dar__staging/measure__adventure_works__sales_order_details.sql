MODEL (
  ...
  tags hook,
  grain (_pit_hook__order_line__sales, _hook__epoch__date),
  references (_pit_hook__order_line__sales, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__order_line__sales,
    sales_order_detail__modified_date::DATE AS measure_date,
    1 AS measure__sales_order_details_modified
  FROM cte__source
  WHERE sales_order_detail__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
