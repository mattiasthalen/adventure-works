...
WITH cte__bridge AS (
  SELECT
    ...
    _hook__epoch__date,
    measure__business_entity_contacts_modified,
  ...
  LEFT JOIN dar__staging.measure__adventure_works__business_entity_contacts USING (_pit_hook__business_entity)
),
cte__pit_lookup AS (
  SELECT
    cte__bridge.peripheral,
    cte__bridge._pit_hook__business_entity,
    uss_bridge__contact_types._pit_hook__reference__contact_type,
    cte__bridge._hook__business_entity,
    cte__bridge._hook__epoch__date,
    cte__bridge.measure__business_entity_contacts_modified,
    GREATEST(
        cte__bridge.bridge__record_loaded_at,
        uss_bridge__contact_types.bridge__record_loaded_at
    ) AS bridge__record_loaded_at,
    GREATEST(
        cte__bridge.bridge__record_updated_at,
        uss_bridge__contact_types.bridge__record_updated_at
    ) AS bridge__record_updated_at,
    GREATEST(
        cte__bridge.bridge__record_valid_from,
        uss_bridge__contact_types.bridge__record_valid_from
    ) AS bridge__record_valid_from,
    LEAST(
        cte__bridge.bridge__record_valid_to,
        uss_bridge__contact_types.bridge__record_valid_to
    ) AS bridge__record_valid_to,
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          cte__bridge.bridge__is_current_record::BOOL,
          uss_bridge__contact_types.bridge__is_current_record::BOOL
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.uss_bridge__contact_types
  ON cte__bridge._hook__reference__contact_type = uss_bridge__contact_types._hook__reference__contact_type
  AND cte__bridge.bridge__record_valid_from >= uss_bridge__contact_types.bridge__record_valid_from
  AND cte__bridge.bridge__record_valid_to <= uss_bridge__contact_types.bridge__record_valid_to
),
