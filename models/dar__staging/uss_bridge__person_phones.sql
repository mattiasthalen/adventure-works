...
WITH cte__bridge AS (
  SELECT
    ...
    _hook__epoch__date,
    measure__person_phones_modified,
  ...
  LEFT JOIN dar__staging.measure__adventure_works__person_phones USING (_pit_hook__person__individual)
),
cte__pit_lookup AS (
  SELECT
    ...
    cte__bridge.measure__person_phones_modified,
    GREATEST(
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          uss_bridge__phone_number_types.bridge__is_current_record
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.uss_bridge__phone_number_types
