...
WITH cte__bridge AS (
  SELECT
    ...
    _hook__epoch__date,
    measure__product_vendors_last_receipt,
    measure__product_vendors_modified,
  ...
  LEFT JOIN dar__staging.measure__adventure_works__product_vendors USING (_pit_hook__vendor)
),
cte__pit_lookup AS (
  SELECT
    ...
    cte__bridge.measure__product_vendors_last_receipt,
    cte__bridge.measure__product_vendors_modified,
    GREATEST(
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          uss_bridge__unit_measures.bridge__is_current_record
        ]
  ...
  LEFT JOIN dar__staging.uss_bridge__unit_measures
