...
WITH cte__bridge AS (
  ...
  LEFT JOIN dar__staging.measure__adventure_works__products USING (_pit_hook__product)
),
cte__pit_lookup AS (
  SELECT
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          uss_bridge__product_subcategories.bridge__is_current_record,
          uss_bridge__product_models.bridge__is_current_record
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.uss_bridge__product_subcategories
  ...
  LEFT JOIN dar__staging.uss_bridge__product_models
