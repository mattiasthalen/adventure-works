...
WITH cte__bridge AS (
  SELECT
    ...
    measure__purchase_order_details_modified,
  ...
  LEFT JOIN dar__staging.measure__adventure_works__purchase_order_details USING (_pit_hook__order_line__purchase)
),
cte__pit_lookup AS (
  SELECT
    ...
    cte__bridge.measure__purchase_order_details_modified,
    GREATEST(
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          uss_bridge__purchase_order_headers.bridge__is_current_record,
  ...
  FROM cte__bridge
  LEFT JOIN dar__staging.uss_bridge__purchase_order_headers
