...
WITH cte__bridge AS (
  ...
  LEFT JOIN dar__staging.measure__adventure_works__sales_order_details USING (_pit_hook__order_line__sales)
),
cte__pit_lookup AS (
  SELECT
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          uss_bridge__sales_order_headers.bridge__is_current_record,
          ...
          uss_bridge__special_offers.bridge__is_current_record
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.uss_bridge__sales_order_headers
  ...
  LEFT JOIN dar__staging.uss_bridge__special_offers
