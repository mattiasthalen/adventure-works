...
WITH cte__bridge AS (
  SELECT
    ...
    _hook__epoch__date,
    measure__sales_territories_modified,
  ...
  LEFT JOIN dar__staging.measure__adventure_works__sales_territories USING (_pit_hook__territory__sales)
),
cte__pit_lookup AS (
  SELECT
    ...
    cte__bridge.measure__sales_territories_modified,
    GREATEST(
    ...
    LIST_HAS_ALL(
      ARRAY[True],
        ARRAY[
          ...
          uss_bridge__country_regions.bridge__is_current_record
        ]
    ) AS bridge__is_current_record
  FROM cte__bridge
  LEFT JOIN dar__staging.uss_bridge__country_regions
