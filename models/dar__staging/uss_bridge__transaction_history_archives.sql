...
WITH cte__bridge AS (
  SELECT
    ...
    _hook__product,
    _hook__epoch__date,
    measure__transaction_history_archives_transaction,
    measure__transaction_history_archives_modified,
  ...
  LEFT JOIN dar__staging.measure__adventure_works__transaction_history_archives USING (_pit_hook__transaction_history_archive)
),
cte__pit_lookup AS (
  SELECT
    ...
    cte__bridge.measure__transaction_history_archives_transaction,
    cte__bridge.measure__transaction_history_archives_modified,
    GREATEST(
