...
WITH staging AS (
  ...
  SELECT
    *,
    ...
    product_category__record_valid_to = @max_ts::TIMESTAMP AS product_category__is_current_record,
    CASE
  ...
  SELECT
    ...
    CONCAT('product_category|adventure_works|', product_category__product_category_id) AS _hook__product_category,
    *
  FROM validity
)
SELECT
  ...
  product_category__product_category_id::VARCHAR,
  product_category__modified_date::VARCHAR,
  product_category__name::VARCHAR,
  product_category__rowguid::VARCHAR,
