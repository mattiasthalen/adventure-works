...
WITH staging AS (
  ...
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY product_category_id ORDER BY product_category__record_loaded_at) AS product_category__record_version,
    CASE
    ...
    COALESCE(
      LEAD(product_category__record_loaded_at) OVER (PARTITION BY product_category_id ORDER BY product_category__record_loaded_at),
      @MAX_TS::TIMESTAMP
