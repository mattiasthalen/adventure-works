MODEL (
  ...
  kind INCREMENTAL_BY_TIME_RANGE(
    time_column product_list_price_history__record_updated_at
  ),
  tags hook,
...
WITH staging AS (
  ...
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY product_list_price_history__product_id ORDER BY product_list_price_history__record_loaded_at) AS product_list_price_history__record_version,
    CASE
    ...
    COALESCE(
      LEAD(product_list_price_history__record_loaded_at) OVER (PARTITION BY product_list_price_history__product_id ORDER BY product_list_price_history__record_loaded_at),
