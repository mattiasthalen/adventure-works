...
WITH staging AS (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY sales_order__sales_order_id ORDER BY sales_order__order_date) AS sales_order__customer_order_sequence,
  ...
  SELECT
    ...
    CONCAT('sales_order|adventure_works|', sales_order__sales_order_id)::BLOB AS _hook__sales_order,
    CONCAT('address|adventure_works|', sales_order__bill_to_address_id)::BLOB AS _hook__address__bill_to,
    CONCAT('credit_card|adventure_works|', sales_order__credit_card_id)::BLOB AS _hook__credit_card,
    CONCAT('currency_rate|adventure_works|', sales_order__currency_rate_id)::BLOB AS _hook__currency_rate,
    CONCAT('customer|adventure_works|', sales_order__customer_id)::BLOB AS _hook__customer,
    CONCAT('sales_person|adventure_works|', sales_order__sales_person_id)::BLOB AS _hook__sales_person,
    CONCAT('ship_method|adventure_works|', sales_order__ship_method_id)::BLOB AS _hook__ship_method,
    CONCAT('address|adventure_works|', sales_order__ship_to_address_id)::BLOB AS _hook__address__ship_to,
    CONCAT('territory|adventure_works|', sales_order__territory_id)::BLOB AS _hook__territory,
    *
  FROM validity
)
SELECT
