...
WITH staging AS (
  SELECT
    business_entity_id AS sales_person__sales_person_id,
  ...
  SELECT
    *,
    ROW_NUMBER() OVER (PARTITION BY sales_person__sales_person_id ORDER BY sales_person__record_loaded_at) AS sales_person__record_version,
    CASE
    ...
    COALESCE(
      LEAD(sales_person__record_loaded_at) OVER (PARTITION BY sales_person__sales_person_id ORDER BY sales_person__record_loaded_at),
  ...
  SELECT
    CONCAT(
      'sales_person|adventure_works|',
      sales_person__sales_person_id,
      '~epoch|valid_from|',
      sales_person__record_valid_from
    )::BLOB AS _pit_hook__sales_person,
    CONCAT('sales_person|adventure_works|', sales_person__sales_person_id)::BLOB AS _hook__sales_person,
    CONCAT('territory|adventure_works|', sales_person__territory_id)::BLOB AS _hook__territory,
    *
  FROM validity
)
SELECT
