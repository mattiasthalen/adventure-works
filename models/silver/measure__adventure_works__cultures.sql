MODEL (
  ...
  tags hook,
  grain (_pit_hook__reference__culture, _hook__epoch__date),
  references (_pit_hook__reference__culture, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__reference__culture,
    culture__modified_date::DATE AS measure_date,
    1 AS measure__cultures_modified
  FROM cte__source
  WHERE culture__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
