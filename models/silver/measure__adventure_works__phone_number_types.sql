MODEL (
  ...
  tags hook,
  grain (_pit_hook__reference__phone_number_type, _hook__epoch__date),
  references (_pit_hook__reference__phone_number_type, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__reference__phone_number_type,
    phone_number_type__modified_date::DATE AS measure_date,
    1 AS measure__phone_number_types_modified
  FROM cte__source
  WHERE phone_number_type__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
