MODEL (
  ...
  tags hook,
  grain (_pit_hook__product_category, _hook__epoch__date),
  references (_pit_hook__product_category, _hook__epoch__date)
);
...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__product_category,
    product_category__modified_date::DATE AS measure_date,
    1 AS measure__product_categories_modified
  FROM cte__source
  WHERE product_category__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
