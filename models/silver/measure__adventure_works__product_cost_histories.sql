...
WITH cte__source AS (
  ...
  SELECT
    _pit_hook__product,
    product_cost_history__start_date::DATE AS measure_date,
    1 AS measure__product_cost_histories_started
  FROM cte__source
  WHERE product_cost_history__start_date IS NOT NULL
), cte__end_date AS (
  SELECT
    _pit_hook__product,
    product_cost_history__end_date::DATE AS measure_date,
    1 AS measure__product_cost_histories_finished
  FROM cte__source
  WHERE product_cost_history__end_date IS NOT NULL
), cte__modified_date AS (
  SELECT
    _pit_hook__product,
    product_cost_history__modified_date::DATE AS measure_date,
    1 AS measure__product_cost_histories_modified
  FROM cte__source
  WHERE product_cost_history__modified_date IS NOT NULL
), cte__measures AS (
  SELECT
    *
