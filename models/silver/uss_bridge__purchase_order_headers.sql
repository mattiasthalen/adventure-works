...
WITH cte__bridge AS (
  ...
  LEFT JOIN silver.measure__adventure_works__purchase_order_headers USING (_pit_hook__order__purchase)
),
cte__pit_lookup AS (
  SELECT
  ...
  LEFT JOIN silver.uss_bridge__vendors
  ...
  LEFT JOIN silver.uss_bridge__product_vendors
