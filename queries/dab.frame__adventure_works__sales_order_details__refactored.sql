...
"raw__adventure_works__sales_order_details" /*                                           
db.das.raw__adventure_works__sales_order_details */ 
  WHERE sales_order_id IN (SELECT sales_order_id FROM cte__upserts)
    AND TO_TIMESTAMP(                                                                        
      CAST("raw__adventure_works__sales_order_details"."_dlt_load_id" AS DECIMAL(18, 3)) 
    ) < '1970-01-01 00:00:00' 
  QUALIFY ROW_NUMBER() OVER (PARTITION BY sales_order_id ORDER BY record_loaded_at DESC) = 1
),"cte__source" AS (                                                                  
  SELECT * FROM cte__upserts
  UNION ALL
  SELECT * FROM cte__previous
), "cte__scd" AS (                                                                       
  SELECT                                                                                 
